...
function BlogMain() {
    const blogs = [
      ...
      {id: 2, title: 'In the woods', text: 'Some quick example text to build on the card title and make up the bulk of the card content.', img: '/static/media/bakery-gd88b1a355_1920.869414c14bf39527e85c.jpg', alt: 'sasas'},
  ...
  return (
    <div className='blog-container'>
        ...
        return (
          <div key={index}>
            <BlogCards imgSrc={blog.img} alt={blog.alt} blogNo={blog.id}  title={blog.title} text={blog.text} bId={blog.id} dataKey={index}/>
          </div>
        );
      })}
    </div>
